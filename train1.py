...
from models.MLFM_Bert import MLFMBertClassifier
...
class Instructor:
    ...
    def __init__(self, opt):
        ...
        if 'bert' in opt.model_name:
            ...
            trainset = ABSAGCNData(opt.dataset_file['train'], tokenizer, opt=opt, dep_vocab=dep_vocab)
            testset = ABSAGCNData(opt.dataset_file['test'], tokenizer, opt=opt, dep_vocab=dep_vocab)
        else:    
...
def main():    
    ...
    dataset_files = {
        ...
        'mams': {
            'train': './dataset/MAMS_corenlp/train.json',
            'test': './dataset/MAMS_corenlp/test.json',
        },
        'restaurant15': {
            'train': './dataset/semeval15/train.json',
            'test': './dataset/semeval15/test.json',
        },
        'restaurant16': {
            'train': './dataset/semeval16/train.json',
            'test': './dataset/semeval16/test.json',
        }
    }
    
    input_colses = {  
        'MLFMbert': ['text_bert_indices', 'bert_segments_ids', 'attention_mask', 'deprel_adj', 'asp_start', 'asp_end','tok2ori_map', 'src_mask', 'aspect_mask']
    }
    ...
    parser.add_argument('--model_name', default='MLFMbert', type=str, help=', '.join(model_classes.keys()))
    ...
    # deprel Hyperparameters
    parser.add_argument('--dep_size', default=46, type=int)
    parser.add_argument('--dep_embed_dim', default=20, type=int)
    parser.add_argument('--dynamic_tree_attn_head', default=5, type=int)
    parser.add_argument('--CrossAttentionFusion_head', default=5, type=int)
    parser.add_argument('--nodalAttention_dropout', type=float, default=0.5, help='adaptiveCrossFusion dropout')
    parser.add_argument('--adaptiveResidualFusion_dropout', type=float, default=0.3, help='adaptiveResidualFusion dropout')
    parser.add_argument('--adaptiveCrossFusion_dropout', type=float, default=0.3, help='adaptiveCrossFusion dropout')
    parser.add_argument('--adaptiveCrossFusion_num_iterations', type=int, default=1, help='adaptiveCrossFusion dropout')
    
    # * bert
    parser.add_argument('--pretrained_bert_name', default='../SSEGCN-ABSA/bert-base-uncased', type=str)
