...
class SentenceDataset(Dataset):
    ...
    def __init__(self, fname, tokenizer, opt, vocab_help):
        ...
        for obj in tqdm(parse(fname), total=len(parse(fname)), desc="Training examples"):
            ...
            if opt.parseadj:
                from absa_parser import headparser # type: ignore
                # * adj
            ...
            deprel_adj = generate_adjacency_matrix(head, deprel)
            data.append({
                'text': text, 
                'aspect': aspect, 
                'post': post,
...
def generate_adjacency_matrix(head, deprel):
    max_size = len(head)  # 根据输入数组的长度确定矩阵大小
    adj_matrix = np.zeros((max_size, max_size), dtype=int)
    # 填充邻接矩阵
    for i in range(max_size):
        if head[i] != 0 and head[i] <= max_size:  # 确保索引不越界
            head_index = head[i] - 1  # head数组从1开始，转换为0开始的索引
            deprel_type = deprel[i]
            adj_matrix[head_index][i] = deprel_type
            
    deprel_adj = np.zeros((100, 100))
    deprel_adj[:len(head), :len(head)] = adj_matrix
